# Runixo v2.0 快速开始指南

欢迎使用Runixo v2.0！本指南将帮助你快速上手新增的核心功能。

---

## 📋 目录

1. [备份管理](#备份管理)
2. [计划任务](#计划任务)
3. [应用商店](#应用商店)
4. [插件开发](#插件开发)

---

## 🔐 备份管理

### 功能概述
自动化备份系统，支持文件、数据库、Docker容器等多种备份类型，可将备份存储到本地或云端。

### 快速开始

#### 1. 创建备份策略

```bash
# 导航到备份管理
应用 → 数据管理 → 备份管理 → 创建备份策略
```

#### 2. 配置备份目标

**文件备份**:
```
类型: 文件/目录
路径: /var/www/html
排除: *.log, *.tmp
```

**数据库备份**:
```
类型: 数据库
数据库名: wordpress
类型: MySQL/PostgreSQL/MongoDB
```

**Docker备份**:
```
类型: Docker容器
容器名: my-app
```

#### 3. 选择存储位置

**本地存储**:
```
类型: 本地存储
路径: /backup
```

**阿里云OSS**:
```
类型: 阿里云OSS
Bucket: my-backup-bucket
Access Key ID: ****
Secret Access Key: ****
区域: oss-cn-hangzhou
```

**AWS S3**:
```
类型: AWS S3
Bucket: my-backup-bucket
Access Key ID: ****
Secret Access Key: ****
区域: us-east-1
```

#### 4. 设置调度

```
调度类型: 每日
时间: 02:00
保留策略: 保留最近7个备份
```

#### 5. 高级选项

```
✅ 压缩: 使用gzip压缩
✅ 加密: AES-256加密
密码: ********
```

### 恢复备份

```bash
1. 在"备份记录"标签页找到要恢复的备份
2. 点击"恢复"按钮
3. 选择恢复目标路径（留空使用原路径）
4. 确认恢复
```

### 常见问题

**Q: 备份失败怎么办？**
```
A: 检查以下几点：
   1. 目标路径是否存在且有权限
   2. 存储空间是否充足
   3. 网络连接是否正常（云存储）
   4. 查看备份日志获取详细错误信息
```

**Q: 如何测试备份是否可用？**
```
A: 建议定期进行恢复测试：
   1. 选择一个备份
   2. 恢复到测试目录
   3. 验证数据完整性
```

---

## ⏰ 计划任务

### 功能概述
强大的任务调度系统，支持Cron表达式和固定间隔，可执行Shell命令、HTTP请求、备份任务等。

### 快速开始

#### 1. 创建Shell任务

```bash
# 导航到计划任务
应用 → 数据管理 → 计划任务 → 创建任务
```

**配置示例**:
```
任务名称: 清理临时文件
任务类型: Shell命令
命令: find /tmp -type f -mtime +7 -delete
工作目录: /tmp
调度: Cron表达式
Cron: 0 3 * * *  (每天凌晨3点)
```

#### 2. 创建HTTP任务

```
任务名称: 健康检查
任务类型: HTTP请求
URL: https://api.example.com/health
方法: GET
调度: 固定间隔
间隔: 300秒 (5分钟)
```

#### 3. 创建备份任务

```
任务名称: 每日数据库备份
任务类型: 备份任务
备份策略: 选择已创建的备份策略
调度: Cron表达式
Cron: 0 2 * * *  (每天凌晨2点)
```

### Cron表达式

#### 常用表达式

```bash
# 每分钟
* * * * *

# 每小时
0 * * * *

# 每天凌晨2点
0 2 * * *

# 每周日凌晨3点
0 3 * * 0

# 每月1号凌晨4点
0 4 1 * *

# 每6小时
0 */6 * * *

# 工作日早上9点
0 9 * * 1-5
```

#### 使用Cron构建器

```bash
1. 点击Cron表达式输入框旁的"构建器"按钮
2. 使用可视化界面设置时间
3. 或选择预设表达式
4. 点击"应用"
```

### 查看执行历史

```bash
1. 切换到"执行历史"标签页
2. 筛选任务和状态
3. 点击"查看输出"查看详细日志
```

### 通知设置

```
✅ 成功时通知
✅ 失败时通知
通知渠道: 邮件/Webhook/系统通知
```

---

## 🏪 应用商店

### 功能概述
一键部署常用应用，支持WordPress、MySQL、Redis、Nginx等，自动配置Docker容器。

### 快速开始

#### 1. 部署WordPress

```bash
# 导航到应用商店
应用 → 扩展 → 应用商店 → WordPress
```

**配置参数**:
```
应用名称: my-wordpress
访问端口: 8080
数据库密码: ********
管理员用户: admin
管理员密码: ********
```

**点击"部署"**，等待部署完成。

**访问应用**: `http://localhost:8080`

#### 2. 部署MySQL

```
应用名称: my-mysql
访问端口: 3306
Root密码: ********
数据库名: mydb
```

#### 3. 部署Redis

```
应用名称: my-redis
访问端口: 6379
密码: ********
```

### 应用管理

#### 启动应用
```bash
已安装应用 → 选择应用 → 启动
```

#### 停止应用
```bash
已安装应用 → 选择应用 → 停止
```

#### 卸载应用
```bash
已安装应用 → 选择应用 → 卸载
```

### 查看应用状态

```bash
已安装应用标签页显示：
- 应用状态（运行中/已停止）
- 资源使用（CPU/内存）
- 访问地址
- 端口映射
```

### 可用应用列表

| 应用 | 类型 | 说明 |
|------|------|------|
| WordPress | Web应用 | 流行的CMS系统 |
| MySQL | 数据库 | 关系型数据库 |
| PostgreSQL | 数据库 | 高级关系型数据库 |
| MongoDB | 数据库 | NoSQL文档数据库 |
| Redis | 缓存 | 内存数据库 |
| Nginx | Web服务器 | 高性能Web服务器 |

---

## 🔌 插件开发

### 功能概述
完整的插件SDK，支持快速开发Runixo插件，扩展应用功能。

### 快速开始

#### 1. 安装CLI工具

```bash
cd packages/plugin-cli
npm install
npm link
```

#### 2. 创建插件

```bash
# 使用交互式向导
runixo-plugin create

# 或指定模板
runixo-plugin create --template=cloud-service my-plugin
```

#### 3. 开发插件

```bash
cd my-plugin
npm install
npm run dev
```

#### 4. 插件结构

```
my-plugin/
├── src/
│   ├── main/
│   │   └── index.ts        # 主进程代码
│   └── renderer/
│       └── views/
│           └── Main.vue    # UI界面
├── plugin.json             # 插件配置
├── package.json
└── tsconfig.json
```

#### 5. 插件示例

```typescript
// src/main/index.ts
import { Plugin, PluginContext } from 'runixo-plugin-sdk'

export default class MyPlugin extends Plugin {
  constructor(context: PluginContext) {
    super(context)
  }

  async onLoad() {
    // 注册菜单
    this.registerMenu({
      id: 'my-plugin-menu',
      label: '我的插件',
      icon: 'Star',
      route: '/plugin/my-plugin',
      position: 'sidebar'
    })

    // 注册路由
    this.registerRoute({
      path: '/plugin/my-plugin',
      name: 'MyPlugin',
      component: './views/Main.vue'
    })

    // 注册工具
    this.registerTool({
      name: 'my_tool',
      displayName: '我的工具',
      description: '这是一个示例工具',
      category: 'custom',
      parameters: {
        input: {
          type: 'string',
          description: '输入参数',
          required: true
        }
      },
      handler: async (params) => {
        return { result: `处理: ${params.input}` }
      }
    })
  }

  async onEnable() {
    this.context.ui.showNotification('插件已启用', 'success')
  }
}
```

#### 6. 构建插件

```bash
npm run build
```

#### 7. 测试插件

```bash
npm test
```

### 插件API

#### 存储API
```typescript
// 保存数据
await this.context.storage.set('key', 'value')

// 读取数据
const value = await this.context.storage.get('key')

// 安全存储（加密）
await this.context.secureStorage.set('api_key', 'secret')
```

#### HTTP API
```typescript
// GET请求
const response = await this.context.http.get('https://api.example.com')

// POST请求
const response = await this.context.http.post('https://api.example.com', {
  data: 'value'
})
```

#### UI API
```typescript
// 显示通知
this.context.ui.showNotification('操作成功', 'success')

// 显示对话框
const result = await this.context.ui.showDialog({
  title: '确认',
  message: '确定要执行此操作吗？',
  type: 'warning'
})

// 显示确认框
const confirmed = await this.context.ui.showConfirm('确定要删除吗？')
```

#### Agent API
```typescript
// 注册Agent工具
this.registerAgentTool({
  name: 'my_agent_tool',
  displayName: '我的Agent工具',
  description: '这是一个Agent工具',
  category: 'custom',
  dangerous: false,
  parameters: {
    type: 'object',
    properties: {
      param: {
        type: 'string',
        description: '参数'
      }
    },
    required: ['param']
  },
  handler: 'myHandler'
})
```

### 更多资源

- 📖 [插件SDK完整文档](./docs/PLUGIN_SDK_DESIGN.md)
- 📖 [API参考](./packages/README.md)
- 💡 [示例插件](./plugins/cloudflare-v2/)

---

## 🆘 获取帮助

### 文档
- [技术文档](./docs/)
- [API参考](./packages/README.md)
- [实施进度](./docs/IMPLEMENTATION_PROGRESS.md)

### 常见问题

**Q: 如何连接到服务器？**
```
A: 设置 → 服务器管理 → 添加服务器
   输入服务器地址、端口和认证令牌
```

**Q: 如何查看日志？**
```
A: 每个功能都有日志查看功能：
   - 备份管理：点击备份记录的"日志"按钮
   - 计划任务：点击执行记录的"查看输出"按钮
   - 应用商店：查看应用详情中的日志
```

**Q: 如何更新应用？**
```
A: 帮助 → 检查更新
   或访问 GitHub Releases 下载最新版本
```

### 反馈问题

如果遇到问题或有建议，请：
1. 查看文档和常见问题
2. 在GitHub上提交Issue
3. 加入社区讨论

---

## 🎉 开始使用

现在你已经了解了Runixo v2.0的核心功能，开始使用吧！

1. ✅ 创建第一个备份策略
2. ✅ 设置第一个计划任务
3. ✅ 部署第一个应用
4. ✅ 开发第一个插件

祝你使用愉快！🚀

---

**版本**: v2.0  
**更新日期**: 2026-02-06  
**文档**: [完整文档](./docs/)
